...
/****************sticky page submenu******************/
	...
	/* LOGO height */
	$('.page-submenu img').height($('.page-submenu').height());
		    ...
		    $headerNavSpace = ($('body[data-header-format="left-header"]').length > 0 && $(window).width() > 1000) ? 0 : $('#header-outer').outerHeight();
		  	...
		  	$offsetHeight = 0;
		  	  ...
		  	  if($('#wpadminbar').length > 0 && $('#wpadminbar').css('position') == 'fixed') $offsetHeight += $('#wpadminbar').height();
					//if($('#header-secondary-outer').length > 0) $offsetHeight -= $('#header-secondary-outer').height();
		  ...
		  Sticky.prototype.createWaypoint = function() {
		    ...
		    this.waypoint = new Waypoint($.extend({}, this.options, {
		      ...
		      handler: $.proxy(function(direction) {
		        ...
		        if(shouldBeStuck) {
							var submenu_log_left = Math.abs(parseInt($(".page-submenu .full-width-content").css('marginLeft')));
							$('.page-submenu .submenu-logo').css('left',submenu_log_left);
		        	this.$element.addClass('no-trans').css('top',$offsetHeight-$('#header-secondary-outer').height()).css('transform','translateY(0)').addClass('stuck');
		        	var $that = this;
		        	...
		        	this.$element.css('top','0').removeClass('stuck');
							$('.page-submenu .submenu-logo').css('left',0);
		...
		$.each($blog_containers,function(i){
				...
				if($blog_containers[i].parent().hasClass('masonry')) {
					...
					if($blog_containers[i].parent().hasClass('masonry') && $blog_containers[i].parent().hasClass('full-width-content')){
						$blog_containers[i].parent().wrap('<div class="full-width-content blog-fullwidth-wrap"> </div>').removeClass('full-width-content').css({'margin-left':'0','width':'1200px'});
